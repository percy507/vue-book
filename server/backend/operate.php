...
if ($way == "manual" and $flag == "in") {
    ...
    $path = "./server/backend/img/$filename.$suffix";
    $imageSrc = $path;
} elseif ($way == "scan") {
...
if ($way == 'scan') {
    if ($flag == 'in') {
        if ($isBookExist) {
            ...
            // 若书籍不存在，那么插入书籍的所有信息
            $needed_prop = "book_title,book_author,book_type,book_cover,rest_number,book_tags";
            $sql = "INSERT INTO $table_name($needed_prop) VALUES('$title','$author','$type','$imageSrc',$number,'$tags')";
